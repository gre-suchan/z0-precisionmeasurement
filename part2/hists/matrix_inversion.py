...
cache_path = '../cache/'
inverse_loc = cache_path + 'E_inverse.txt'
inverse_err_loc = cache_path + 'E_inverse_errors.txt'


def toy_experiments():
    """Runs a large amount of toy experiments to calculate the error on the
    efficiencies.
    This method also caches its files in ../cache
    :returns: A tuple (inverse_means, inverse_errors, hists) from the toy
    experiments.
    ...
    # Create a list consisting only of the same matrix mat and err_mat,
    # repeated ntoy times
    ...
    # These matrices will hold the estimated errors and means (i think the
    # latter are not that useful?)
    ...
    for j in range(0, 4, 1):
        ...
        for k in range(0, 4, 1):
            ...
            # Based on those, create normalized hists with a certain range
            # within the 4 x 4 matrix
    ...
    if not isdir(cache_path):
        mkdir(cache_path)
    np.savetxt(inverse_loc, np.linalg.inv(mat))
    np.savetxt(inverse_err_loc, inverse_errors)

    return (inverse_means, inverse_errors, hists)


def get_cached_inverse():
    """Gets the inverse matrix and its error from cache if it exists and
    generates it otherwise

    :returns: a tuple (inverse, inverse_err)

    """
    if not isfile(inverse_loc) or not isfile(inverse_err_loc):
        toy_experiments()
    return (np.loadtxt(inverse_loc), np.loadtxt(inverse_err_loc))


def get_exact_errors():
    """For comparison: Calculate the errors using the exact method from the
    paper (should be eq.s 7 and 9)

    :returns: exact_inverse_errors
...
if __name__ == "__main__":
    inverse_means, inverse_errors, _ = toy_experiments()
